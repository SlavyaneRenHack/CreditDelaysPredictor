...
# def prepare_base_df(user_df, credit_df, card_df):
...
def prepare_base_df(user_df, credit_df, card_df):
    ...
    def mounth_until_end(open_date, term):
        mounth_passed = (current_date.year - open_date.year) * 12 + current_date.month - open_date.month
        remain_months = term - mounth_passed
        return max(0, remain_months)

    credit_df["REMAIN_MONTHS"] = credit_df.apply(lambda row: mounth_until_end(row['VALUE_DT'], row['TERM']), axis=1)
    credit_df = credit_df.rename(columns={"OVERDUE_IND": "delay"})
    ...
    # Merging
    ## Preparing dataframes
    df_paid_off_counts = credit_df[credit_df["REMAIN_MONTHS"] == 0].groupby("CLIENT_ID").size().reset_index(name="paid_off_count")
    df_active_counts = credit_df[credit_df["REMAIN_MONTHS"] > 0].groupby("CLIENT_ID").size().reset_index(name="active_count")
    df_active_sums = credit_df[credit_df["REMAIN_MONTHS"] > 0].groupby("CLIENT_ID")["ORIG_AMOUNT"].sum().reset_index(name="active_sum")
    df_max_term = credit_df.groupby('CLIENT_ID')['TERM'].max().reset_index(name='max_term')
    df_delay_counts = credit_df[credit_df['delay'] > 0].groupby("CLIENT_ID").size().reset_index(name="credits_delay_count")
    ...
    # Ensure all columns in base_df_columns are present in the DataFrame
    for column in base_df_columns:
        if column not in df.columns:
            df[column] = 0
...
def create_user_data(row):
	return {
		...
		"IP_FLAG": row['IP_FLAG'],
		"SME_FLAG": row['SME_FLAG'],
		"EMPLOYEE_FLAG": row['EMPLOYEE_FLAG'],
		"REFUGEE_FLAG": row['REFUGEE_FLAG'],
		"PDN": row['PDN'],
	}
...
def gen_file():
	...
	model = joblib.load("params/rf_model.joblib")
	user_df = pd.read_excel("data/val.xlsx")

	base_df = prepare_base_df(user_df, credit_df, card_df).fillna(0)
	print(base_df.info())
	print(base_df[["credits_delay_count", "active_count", "active_sum", "max_term"]].describe())

	# print(set(base_df['CLIENT_ID'].values).intersection(set(credit_df['CLIENT_ID'].values)))
	# print(set(base_df['CLIENT_ID'].values).intersection(set(card_df['CLIENT_ID'].values)))
	...
	# 	user_data = create_user_data(row)
	# 	base_df = prepare_base_df(user_data, credit_df, card_df)
		# pred = model.predict_proba(base_df.drop(columns=["CLIENT_ID"]))[0][0] * 100
